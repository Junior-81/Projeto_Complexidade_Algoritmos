...
from app.services.uber_segment_aggregator import UberSegmentAggregator
...
class PathReconstructor:
    ...
    def _build_edge(
        ...
        esforco = self.cost_calc.calculate_effort_score(
            modal, distance_km, climate_factor=climate_factor
        )
    ...
    def _finalize_segment(self, group: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        risco_medio = (
            sum(float(e.get("risco", 0.0)) for e in group) / len(group) if group else 0.0
        )
    ...
    def _build_summary(self, edges: list[dict[str, Any]]) -> dict[str, Any]:
        """Calcula metricas totais a partir das arestas percorridas."""
        ...
        risco_medio = (
            sum(float(e.get("risco", 0.0)) for e in edges) / len(edges) if edges else 0.0
        )
    ...
    def reconstruct_route(self, path: list[tuple[int, str]]) -> dict[str, Any]:
        """Reconstroi rota em formato detalhado e agregado."""
        ...
        for i in range(len(path) - 1):
            ...
            edges.append(self._build_edge(node_curr, node_next, modal_next, selected_edge))

        edges = UberSegmentAggregator(self.cost_calc, self.rain_factor).apply(edges)
        edges = self._apply_bus_fare(edges)
        segments = self._build_segments(edges)
        resumo = self._build_summary(edges)

        return {"edges": edges, "segments": segments, "resumo": resumo}

    # Tarifa unica por EMBARQUE (nao por trecho entre paradas). No roteamento o
    # custo por aresta de onibus e 0; aqui cobramos a tarifa uma vez por trecho
    # continuo de onibus, atribuindo-a a ultima aresta do trecho.
    BUS_FARE = 4.50

    def _apply_bus_fare(self, edges: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Cobra a tarifa fixa uma vez por trecho continuo de onibus."""
        i, n = 0, len(edges)
        while i < n:
            if edges[i].get("modo") != "bus":
                i += 1
                continue
            j = i
            while j < n and edges[j].get("modo") == "bus":
                edges[j]["custo"] = 0.0
                j += 1
            edges[j - 1]["custo"] = round(self.BUS_FARE, 2)
            i = j
        return edges

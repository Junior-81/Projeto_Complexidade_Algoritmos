...
class DijkstraMultimodal:
    ...
    def search(
        ...
        while self.open_set and iteration < max_iterations:
            ...
            for neighbor in self.graph.neighbors(node_curr):
                for _, edge_data in self.graph[node_curr][neighbor].items():
                    ...
                    # Penalidade de embarque: ao subir no onibus (vindo de
                    # nao-onibus) soma um custo fixo. Minimiza o numero de
                    # embarques -> evita o "sobe-desce-sobe" na mesma linha.
        ...
        logger.warning("Dijkstra nao encontrou caminho apos %d iteracoes", iteration)
        return None
